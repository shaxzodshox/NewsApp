package com.shlsoft.newsapp.ui.view_model
...
import com.shlsoft.newsapp.app.NewsApplication
...
import com.shlsoft.newsapp.util.Resource
import kotlinx.coroutines.launch
import retrofit2.Response
import java.io.IOException

class NewsViewModel(app: Application, val newsRepository: NewsRepository) :
    AndroidViewModel(app) { //AndroidViewModel is same as ViewModel but it has a function getApplication which means applicationContext
        ...
        if (response.isSuccessful) {
            response.body()?.let { resultResponse ->
                ...
                if (breakingNewsResponse == null) {
                    //if it is first response ever
                    breakingNewsResponse = resultResponse
                } else {
                    //if we loaded more than one page already
    ...
    private suspend fun safeBreakingNewsCall(countrCode: String){
        breakingNews.postValue(Resource.Loading())
        try {
            if (hasInternetConnection()){
                val response = newsRepository.getBreakingNews(countrCode, breakingNewsPage)
